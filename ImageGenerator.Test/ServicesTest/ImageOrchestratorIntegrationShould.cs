    ...
    public class ImageOrchestratorIntegrationShould
    {
        ...
        public void Creates_Folder_Resize_And_Save_Image()
        {
            ImageOutputProperties imageOutputProperties = new ImageOutputPropertiesBuilder().
                ...
                WithFormat(Abstractions.ImageEncoder.Png);
            ...
            string outputImagePath = Path.Combine(Directory.GetCurrentDirectory(), "output", "image.png");
            Assert.True(File.Exists(outputImagePath));
        }


        [Fact]
        public void Creates_Multiple_Folder_Resize_And_Save_Image()
        {
            ImageOutputProperties imageOutputProperties = new ImageOutputPropertiesBuilder().
                WithRelativeFolderPath("output").
                WithResolution(new Size(20, 20)).
            ...
            string outputImagePath = Path.Combine(Directory.GetCurrentDirectory(), "output", "image.png");
            string otherOutputImagePath = Path.Combine(Directory.GetCurrentDirectory(), "output2", "image.png");
        ...
        public void Creates_Multiple_Files_With_Different_Name_In_Same_Folder()
        {
            ...
            string outputImagePath = Path.Combine(Directory.GetCurrentDirectory(), "output3", "image.png");
            string outputImagePathWithSuffix = Path.Combine(Directory.GetCurrentDirectory(), "output3", "imageDummy.png");
        ...
        {
            return File.ReadAllBytes(Path.Combine("Resources", "Dummy_image.png"));
        }

        private string GivenAPath(string fileName)
        {
            return string.Concat(Directory.GetCurrentDirectory(), Path.DirectorySeparatorChar, fileName);
        }
    }
}
